...
Figura generateSurface(const char* patch_file, int tesselation) {
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            U_M[i] += u_array[j] * M[i][j];
        }
    }
	
    // leitura do ficheiro patch
	//linhas     16 pontos     3 coordenadas
    ...
    for (std::vector<std::vector<float>> patch : patches) {
		...
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
				for (int k = 0; k < 3; ++k) {
					std::cout << ponto_base_Transposta_V[i]<< std::endl;
					std::cout << '\n' << std::endl;
					ponto_base_Transposta_V[i]+= pontos_base[i][j][k] * Transposta_V[j];
				}
			}
		}
		...
		float result[4];
		for (int i = 0; i < 4; ++i) {
				result[i] += ponto_base_Transposta_V[i] * U_M[i];
